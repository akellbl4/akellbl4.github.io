(function() {
    try {
        const wrapNameLetters = name => (name.split('').map(item => item !== ' ' ? `<span>${item}</span>` : ' ').join(''))
        const generateKerning = () => {
            const kerningSize = [ 55, 45, 45, 45, 0, 35, 4, 35, 1, 55 ];
            const kerningTemplate = (value, i) => `.my-name-is span:nth-child(${i}){margin-right:-.${value}vmin}`;
        ...
        const nameContainer = document.querySelector('.my-name-is');
